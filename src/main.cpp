...
void handlePINResult(const char *pin) {
  ...
  if ( strcmp(pin,payment_pin.c_str()) == 0 ) {
    payment_pin = "";
    beerScreen();
    return;
  }
...
void configureSwitches() {
  ...
  if ( webSocket.isConnected() ) {
    setUIStatus(PSTR("Ready (online)"),PSTR("Ready to Serve (online)"));
  } else if ( productConfig.getNumProducts() > 0 ) {
    setUIStatus(PSTR("Ready (offline)"),PSTR("Ready to Serve (offline)"));  
  } else {
    setUIStatus(PSTR("Not connected"),PSTR("Not connected"));    
  }
}
  ...
  switch(type) {
    case WStype_DISCONNECTED:      
      if ( WiFi.status() != WL_CONNECTED ) {
        ...
        webSocket.disableHeartbeat();
        if ( productConfig.getNumProducts() > 0 ) {
          setUIStatus(PSTR("Ready (offline)"),PSTR("WiFi Disconnected"));
        } else {
...
#endif
        if ( productConfig.getNumProducts() > 0 ) {
          setUIStatus("Ready (offline)","WebSocket Disconnected");        
        } else {
...
#endif
      if ( productConfig.getNumProducts() > 0 ) {
        setUIStatus("Ready (offline)","SSID not found");
      } else {
